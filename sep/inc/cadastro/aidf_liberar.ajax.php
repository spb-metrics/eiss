...
$sql_liberado = mysql_query("SELECT confirmar FROM aidf_solicitacoes WHERE codigo = '$codaidf'");
list($aidf_estado) = mysql_fetch_array($sql_liberado);

$sql_aidf = mysql_query("
				SELECT 
					emissor.razaosocial AS emissor_razao,
					if(emissor.cnpj <>'', emissor.cnpj , emissor.cpf) AS emissor_cnpj,
					emissor.inscrmunicipal AS emissor_im,                            						
					emissor.logradouro AS emissor_endereco,                       	
					emissor.municipio AS emissor_municipio,
					emissor.uf AS emissor_uf,							
					grafica.razaosocial AS grafica_razao,
					if(grafica.cnpj <>'', grafica.cnpj , grafica.cpf) AS grafica_cnpj,														
					grafica.inscrmunicipal AS grafica_im,										
					grafica.logradouro AS grafica_endereco,							
				...
				INNER JOIN 
					cadastro AS emissor ON aidf.codemissor = emissor.codigo
				INNER JOIN 
					cadastro AS grafica ON aidf.codgrafica = grafica.codigo
				INNER JOIN 
    ...
    <table width="100%">
		...
		<tr>
			<td align="right" colspan="6">
				<input type="submit" value="Imprimir segunda via" name="btSegundaVia" class="botao"
            	onclick="cancelaAction('frmAidf','inc/cadastro/aidf_imprimir.php','_blank');">
			</td>
		</tr>
		<?php
			}else{
        ...
        <tr>	
            <td align="right" width="38%">
            	<input type="button" value="Liberar" name="btLiberar"  class="botao"
            	onclick="alert('Liberado com sucesso!');acessoAjax('inc/cadastro/aidf_liberar.php','frmAidf','divaidf');">     	
            
            	
            </td>
            <td align="left" colspan="6">
            	<?php
					$crypto_aidf = base64_encode($codaidf);
				?>
            	<input type="submit" value="Liberar e Imprimir" name="btLiberar" class="botao"
            	onclick="alert('Liberado com sucesso!');window.open('inc/cadastro/aidf_imprimir.php?txtCodAidf=<?php echo $crypto_aidf;?>');acessoAjax('inc/cadastro/aidf_liberar.php','frmAidf','divaidf');">
            </td>
        </tr>
		<?php
			}
