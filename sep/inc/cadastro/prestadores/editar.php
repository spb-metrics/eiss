...
$sql_dados_empresa=mysql_query("SELECT nome,codtipo,codtipodeclaracao,razaosocial,$campo,inscrmunicipal,email,logradouro,numero,estado,nfe, pispasep FROM cadastro
 WHERE codigo = '$codigoempresa'");
list($Nempresa,$CodTipo,$CodTipoDec,$Rempresa,$CNCPempresa,$Iempresa,$EMempresa,$ENempresa,$NMempresa,$OPestado,$Nfe,$Pispasep)=mysql_fetch_array($sql_dados_empresa); 
...
if(($nomeempresa != $Nempresa) ||($razaosocial != $Rempresa)|| ($cnpjcpfempresa != $CNCPempresa) || ($enderecoempresa != $ENempresa) ||($incmunicipal != $Iempresa) || ($emailempresa != $EMempresa) || ($estado != $OPestado) || ($codtipodeclaracao != $CodTipoDec) || ($codtipo_prestador != $CodTipo) || ($codbanco != $Codbanco_Inst) || ($codbanco != $Codbanco_Opr) || ($agencia != $Agencia_Inst) || ($agencia != $Agencia_Opr) || ($adm_publica != $Admpublica_Cart) || ($nivel != $Nivel_Cart) || ($nfe != $Nfe) || ($pispasep != $Pispasep))
 {   
   ...
   $sql=mysql_query("
   UPDATE cadastro SET nome = '$nomeempresa', razaosocial = '$razaosocial', $campo = '$cnpjcpfempresa',
   inscrmunicipal = '$inscrmunicipal', email= '$emailempresa',logradouro = '$enderecoempresa', numero = '$numero', estado = '$estado', uf='$uf',
   municipio='$municipio', nfe='$nfe', complemento = '$complemento', bairro='$bairro', cep='$cep', fonecomercial = '$fone', fonecelular = '$celular', codtipodeclaracao='$codtipodeclaracao', codtipo = '$codtipo_prestador', pispasep = '$pispasep' WHERE codigo = '$codigoempresa'");  
