...
<?php
	...
	if(isset($_GET["hdCodSolicitacao"])){
		...
		mysql_query("UPDATE graficas SET estado = 'A' WHERE codigo = '$codigo'");
		add_logs('Liberou uma Solicitação de uma Gráfica');
	...
	$query = ("SELECT codigo, nome, cnpjcpf FROM graficas WHERE estado = 'NL' $string ORDER BY nome");
	$sql_pedidos = Paginacao($query,'frmLiberarGraficas','divlistaGraficas',15);
...
?>
	<table width="100%">
		...
		<tr bgcolor="#FFFFFF">
			...
			<td align="center">
				<input name="btLiberar" type="button" value="Ativar" class="botao" onClick="document.getElementById('hdCodSolicitacao').value=<?php echo $codigo;?>;acessoAjax('inc/prestadores/liberar_graficas_lista.ajax.php','frmLiberarGraficas','divlistaGraficas');alert('Cadastro ativado')">
			</td>
		</tr>
		<?php
			}//fim while
