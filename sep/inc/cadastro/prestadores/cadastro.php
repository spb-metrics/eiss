	...
	if(($_POST['CODEMISSOR'])){		   
		...
		$sql=mysql_query("
						...
						WHERE
							codigo='$codigo'
						");
		list($codigo,$codtipo,$codtipodec,$nome,$razaosocial,$cnpjcpf,$inscrmunicipal,$logradouro,$numero,$complemento,$bairro,$fone,$celular,$cep,$municipio,$uf,$logo,$email,$ultima,$notalimite,$estado,$codcontador,$nfe,$pispasep)= mysql_fetch_array($sql);	
...
<table border="0" cellspacing="0" cellpadding="0" class="form">
	...
	<form  method="post" name="frmCadastroEmpresa" id="frmCadastroEmpresa">
		...
		<table border="0" align="center" id="tblEmpresa">
			<tr>
				...
				<td colspan="3">
					<select name="cmbCodtipo" id="cmbCodtipo" class="combo" onchange="alternaCampos('cmbCodtipo')">
						<option value=""></option>
						<?php
							$sql_codtipo = mysql_query("SELECT codigo, tipo, nome FROM tipo WHERE tipo <> 'simples' ORDER BY nome");
			...
			<tr>
			    ...
			    <td align="left">
					...
					value="<?php if(isset($complemento)){echo $complemento;} ?>" />				</td>
			</tr>
            <tr>
			...
			<tr>
				...
				<td colspan="3" align="left">
					<label><input type="checkbox" value="S"  name="txtNfe" id="txtNfe" <?php if(($nfe == 'S') || ($nfe == "s")){echo "checked=\"checked\"";} ?>/>
					<em>Esta empresa emite Nota Fiscal eletr&ocirc;nica</em></label></td>
			</tr>
			<?php if($_POST['CODEMISSOR']){?>
			<tr>
		...
		<table id="bigtable">
			...
			<tr>
				<td colspan="4" align="right"><font color="#FF0000">*</font> Campos Obrigat&oacute;rios<br /></td>
			</tr>
		</table>
		</fieldset>
