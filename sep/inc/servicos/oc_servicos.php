...
<?php
	...
	if($_POST['COD']){
		include("oc_servico_editar.php");
		
	}else{
		if($_POST['excluir']){
			$codigo = $_POST['excluir'];
			//Mensagem('Serviço excluido!');
			mysql_query("DELETE FROM dop_contas WHERE codigo='$codigo'");
			add_logs('Excluiu uma conta de Operadora de Crédito');
			Mensagem('Serviço excluído!');
			unset($codigo);
		}
	?>
	<form name="frmBusca" method="post" id="frmBusca" onsubmit="acessoAjax('inc/servicos/oc_servico_busca.ajax.php','frmBusca','dvResult'); return false;">
