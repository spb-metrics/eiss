...
//seleciona os dados da autuacao: numero do processo, ano do processo, razao social, codigo da autuacao, descricao da autuacao, nomero e ano da infracao, descricao da infracao, fundamentacao legal (legislacao municipal, como leis e multas)
	list($nroprocesso, $anoprocesso, $razaosocial, $autuacao_nro, $autuacao_ano, $autuacao_descricao, $autuacao_historico, $infracoes_nro, $infracoes_ano, $infracoes_titulo, $infracoes_descricao, $fundamentacaolegal, $codautuacao, $reincidencia, $quantidade, $multa) = mysql_fetch_array($sql); 	
?>
...
<fieldset style="margin-left:7px; margin-right:7px;">
	<table>
		...
		<tr>
			<td>Histórico da Infracao:</td>
			<td><?php echo "$autuacao_historico"; ?></td>
		</tr>
		<tr>
			<td>Descri&ccedil;&atilde;o Infracao:</td>
	...
	<table>
		<tr>
			<form method="post" id="frmVoltar">
				...
				<input type="hidden" name="txtAcao" id="txtAcao_frmVoltar" />
				...
				<td><input name="btnVoltar" value="Voltar" type="submit" class="botao" onclick="document.getElementById('txtAcao_frmVoltar').value='documentosautuacao';" /></td>
				<td>
					<?php
        ...
        <form method="post" action="principal.php">
				<input type="hidden" name="include" id="include" value="<?php echo  $_POST['include'];?>" />
				<input type="hidden" name="txtAcao" id="txtAcao2" value="docsprevia" />
				<td><input name="btCancelar" value="Cancelar Autuação"  type="submit" class="botao" onclick="return Confirma('Deseja Cancelar essa Autuacao?')"/></td>
		<?php if($autuacao_descricao == "Notificação"){?>
			...
			<form method="post" action="inc/fiscalizacao/processos/docs_autuacao_imprimir.php" target="_blank">
