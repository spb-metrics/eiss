	...
	if($_POST["btEnviar"]=="Enviar")
		{
			...
			$cpfcnpjprestador=$_POST["txtPrestador"];
			...
			");
			add_logs('Inseriu um Processo Fiscal com Homologação de Tomador');
	...
	elseif($_POST["btHomologar"]=="Homologar")
		{
			...
			add_logs('Autalizou um Processo Fiscal com Homologação de Tomador');
...
?>
	<table width="100%">
		<tr>
			<td colspan="2"><b>Homologação de serviços tomados</b></td>
		</tr>
		<tr>
	...
	<form method="post">
		...
		<input type="hidden" name="txtServicos" value="tomados" />
		<table width="100%">	
			<tr>
				...
				<td>
					<select name="cmbLS" style="width:100%">
						<?php
							$sql=mysql_query("SELECT codigo, descricao FROM servicos");
		...
		<table width="100%">
			...
			<tr>
				<td width="25%">CPF/CNPJ Prestador:</td>
				<td width="25%">
					<input type="text" class="texto" name="txtPrestador" onkeydown="stopMsk( event );" onkeyup="CNPJCPFMsk( this );return NumbersOnly( event );" maxlength="18" />
				</td>
				<td width="25%">Situação do tributo:</td>
				<td width="25%">
			...
			<tr>
				...
				<td width="25%"><input type="text" class="texto" name="txtValorTotal" id="txtValorTotal" onblur="CalculaIss('txtValorTotal','txtIss')" onkeyup="MaskMoeda(this);return NumbersOnly(event);"  /></td>
				<td width="25%">ISS</td>
				<td width="25%"><input type="text" class="texto" name="txtIss" id="txtIss" onkeyup="MaskMoeda(this);return NumbersOnly(event);" /></td>
			</tr>
			<tr>
				<td width="25%">ISS Retido:</td>
				<td width="25%"><input type="text" size="5" class="texto" name="txtIssRetido" id="txtIssRetido" onblur="CalculaIssRetido('txtValorTotal','txtIssRetido')" />%</td>
				<td width="25%">Dedução:</td>
				<td width="25%"><input type="text" class="texto" name="txtDeducao" id="txtDeducao" onblur="SomaDeduc('txtDeducao','txtValorTotal')" onkeyup="MaskMoeda(this);return NumbersOnly(event);" /></td>
			</tr>
			<tr>
				<td width="25%">Local da Prestação de Serviço:</td>
				<td width="25%"><input type="text" class="texto" name="txtLocalPS" /></td>
				<td width="50%" colspan="2"><input type="submit" class="botao" name="btEnviar" value="Enviar" /></td>
			</tr>
		</table>
		<table>
			<tr>
	...
	<table width="100%">
		...
		<?php
			while(list($codhomologacao,$nrodoc,$competencia,$cpfcnpjprestador,$vtotal,$iss,$issretido,$deducao)=mysql_fetch_array($sql))
				{
