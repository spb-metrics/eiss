...
function Uploadimagem($campo,$destino,$cod=NULL){
	if($campo != ""){
		...
		if(array_search($imagem['extensao'], $extpermitidas) === false){
			Mensagem("Por favor, envie arquivos com as seguintes extensões: jpeg, jpg");
		}else{
			...
			/*//recebe o caminho
			$imagem['caminho'] = $imagem['destino'].$imagem['nome'];
			//Cria uma imagem de origem
			$imagem['img_orig'] = imagecreatefromjpeg($imagem['caminho']);
			
			$imagem['ponto_x'] =   imagesx($imagem['img_orig']); 
			$imagem['ponto_y'] =   imagesy($imagem['img_orig']);  
			//calcula o fator
			$imagem['fator'] = ($imagem['ponto_y']/$imagem['ponto_x']);		
			//calcula a altura
			$imagem['altura'] = (100*$imagem['fator']);
			//melhora a definicao da imagem
			$imagem['image_fim'] = imagecreatetruecolor (100,$imagem['altura']); 
			//trata a imagem
			imagecopyresampled($imagem['image_fim'], $imagem['img_orig'], 0, 0, 0, 0, 100,$imagem['altura'], $imagem['ponto_x'], $imagem['ponto_y']);
			//substitui a imagem temporaria pela imagem ja tratada
			imagejpeg($imagem['image_fim'], $imagem['destino'].$imagem['nome']);
			//destroi a imagem de origem
			imagedestroy($imagem['img_orig']);
			//destroi a imagem final
			imagedestroy($imagem['image_fim']);*/
			//retorna o nome da imagem para que se possa salvar no banco
			return $imagem['nome'];
		}
	}else{
