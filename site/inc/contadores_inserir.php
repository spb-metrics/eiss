...
if(($nome !="") && ($razaosocial !="") && ($endereco !="") && ($municipio != "") && ($uf != "") && ($txtNomeSocio1 != "") && ($txtCpfSocio1 != "") && ($cpfcnpj !="") && ($email !="") && ($txtSenha !="") && ($txtSenhaConf == $txtSenha )&&($cmbCodigo!="")) {
	...
	if((mysql_num_rows($sql_teste1)>0)||(mysql_num_rows($sql_teste2)>0))
		{
			echo "
				<script>
					alert('Já existe um contador cadastrado com esse CNPJ / CPF');
	...
	endereco='$endereco', inscrmunicipal='$inscricaomunicipal',  municipio ='$municipio', email='$email',uf='$UF', senha='$txtSenha', simplesnacional = '$simplesnacional', tipo='contador'");
	
	// BUSCA CONTADOR INSERIDO
	$sql_contador = mysql_query("SELECT codigo FROM cadastro_emissores WHERE nome = '$nome'");		
	list($CODCONTADOR) = mysql_fetch_array($sql_contador);
   ...
   //insere os socios no banco
	while($contsocios < $nrosocios) {   
   		if($vetor_sociosnomes[$contsocios] != "") { 	    
			$sql = mysql_query("INSERT INTO cadastro_emissores_socios SET codemissor='$CODCONTADOR', nome = '$vetor_sociosnomes[$contsocios]', cpf = '$vetor_socioscpf[$contsocios]'");	
		} // fim if	
		$contsocios++;
   } // fim while   
	...
	// gera o comprovante em pdf   
   	print "<script language=JavaScript> alert('Cadastro feito com sucesso!!');window.open('contadores_gerapdf.php?CODCONTADOR=$CODCONTADOR');</script>"; 
