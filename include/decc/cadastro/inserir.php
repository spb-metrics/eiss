...
<?php 
   ...
   // Pega as variaveis que vieram por POST
	...
	if(mysql_num_rows($teste_nome)>0){
		...
		2- Clique no link Empreiteiras<br>
		...
		$sql_empresa = mysql_query("SELECT codigo FROM cadastro WHERE cnpj = '$cnpj' ORDER BY CODIGO DESC LIMIT 0,1");
			 ...
			 while(list($codcategoria)=mysql_fetch_array($sql_categoria)) {
			   ...
			   for($conts=1;$conts<=5;$conts++) {    
					...
					if($_POST['cmbCodigo'.$codcategoria.$conts]){
						mysql_query("INSERT INTO cadastro_servicos
	   ...
	   //insere os socios no banco
		while($contsocios < $nrosocios) {
			if($vetor_sociosnomes[$contsocios] != "") {
				...
				mysql_query("INSERT INTO cadastro_resp
        ...
        echo "<script>window.open('../../../reports/decc_comprovante_cadastro.php?COD=$COD');</script>";

        print "<script language=JavaScript> alert('Cadastro feito com sucesso!!');window.location='../../../site/decc.php'</script>";
	} 
?>
