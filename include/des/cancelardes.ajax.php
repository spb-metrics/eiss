...
?>
	...
	$stringSql = ("
		...
		INNER JOIN
			des_servicos ON des_servicos.coddes = des.codigo
		WHERE
			cadastro.cnpj= '$emissor_CNPJ' OR 
			cpf = '$emissor_CNPJ' AND
			(des.estado ='C' OR des.estado='B')
			
		GROUP BY 
			des.codigo
		ORDER BY des.competencia DESC
	");
	...
	if(mysql_num_rows($sql_guias)){
		?>
			  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="1" bordercolor="#CCCCCC" bgcolor="#FFFFFF">
				...
				while(list($codigo, $dataemissao, $total,$valoriss,$competencia,$notas,$estado) = mysql_fetch_array($sql_guias)){
					...
					?>                
	 				<tr id="trDes<?php echo $cont;?>" <?php echo "style=\"$style\"";?>>
						<td align="center">
					  	...
					  	<td align="center">
                      		<input name="txtValorTotal<?php echo $cont;?>" type="text" class="texto" id="txtValorISS<?php echo $cont;?>" value="<?php echo DecToMoeda($valoriss); ?>" size="10" readonly="readonly" style="text-align:right" />                      
                      	</td>
                      	<td align="center">
						...
						<td align="center" id="tdCancelar<?php echo $cont;?>">
	                  	<?php if($estado=='C')echo 'Canc'; else{?>
	                  		<input name="imgCancelar<?php echo $cont;?>" id="imgCancelar<?php echo $cont;?>" type="image" src="img/botoes/botao_cancelar.jpg" onClick="return des.cancelarDeclaracao('<?php echo $cont;?>');">	                  
	                  <?php }?>
	                  	</td>
	                </tr>
		<?php
