...
?>
	...
	$stringSql = ("
		SELECT
			...
			gp.valor, 
			gp.nossonumero, 
			DATE_FORMAT(dif_des.competencia,'%d/%m/%Y'), 
			gp.estado 
		FROM 
			dif_des 		
		INNER JOIN 
			guias_declaracoes as gd ON gd.codrelacionamento=dif_des.codigo
		INNER JOIN 
			guia_pagamento as gp ON  gp.codigo = gd.codguia
		INNER JOIN 
			cadastro as cad  ON  cad.codigo=dif_des.codinst_financeira
		WHERE
			gd.relacionamento = 'dif_des' AND gp.pago='N' AND (cad.cnpj= '$emissor_CNPJ' OR cad.cpf = '$emissor_CNPJ')
		GROUP BY 
			gp.codigo
	");
	 				...
	 				<tr id="trDes<?php echo $cont;?>" <?php echo "style=\"$style\"";?>>
                      ...
                      <td align="center">
                        ...
                        <?php if($estado=='C')echo 'Cancelado'; else{?>
	                  		<input name="imgCancelar<?php echo $cont;?>" id="imgCancelar<?php echo $cont;?>" type="image" src="img/botoes/botao_cancelar.jpg" onClick="return dif.cancelarGuia('<?php echo $cont;?>');">	                  
						<?php };?>
	                  	</td>
	                </tr>
		<?php
