...
?>
	...
	$stringSql = ("
		...
		FROM 
			dop_des
		INNER JOIN 
			guias_declaracoes as gd ON gd.codrelacionamento = dop_des.codigo
		INNER JOIN 
			guia_pagamento as gp ON  gp.codigo = gd.codguia
		INNER JOIN 
			cadastro as cad  ON  cad.codigo = dop_des.codorgaopublico
		WHERE
			gd.relacionamento = 'dop_des' AND gp.pago='N' AND (cad.cnpj= '$emissor_CNPJ' OR cad.cpf = '$emissor_CNPJ')
		GROUP BY 
			gp.codigo
	");
	 				...
	 				<tr id="trDes<?php echo $cont;?>" <?php echo "style=\"$style\"";?>>
                      ...
                      <td align="center">
                        ...
                        <?php if($estado=='C')echo 'Cancelado'; else{?>
	                  		<input name="imgCancelar<?php echo $cont;?>" id="imgCancelar<?php echo $cont;?>" type="image" src="img/botoes/botao_cancelar.jpg" onClick="return dop.cancelarGuia('<?php echo $cont;?>');">	                  
						<?php };?>
	                  	</td>
	                </tr>
		<?php
