...
<?php
	...
	$buscatipo=codtipo('prestador');
	
	$sql = mysql_query("
		SELECT nome, estado, senha FROM cadastro WHERE (cnpj = '$login' OR cpf = '$login') AND codtipodeclaracao=(SELECT codigo FROM declaracoes WHERE declaracao='MEI') 
	");
	if(mysql_num_rows($sql)) { 
		...
		print("<script language=JavaScript>alert('CPF/CNPJ não cadastrado no sistema ou não optante pelo MEI! Favor verificar usuario.');parent.location='../login.php';</script>");
	} 
?>