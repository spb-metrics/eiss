...
$sql_operadora = mysql_query("
	...
	WHERE 
		cnpj = '$operadora_CNPJ'
");
$dados_operadoras = mysql_fetch_array($sql_operadora);

	?>

	<form id="frmGuia" method="post" name="frmDesSegundaVia" action="include/doc/gerasegundavia_guia.php" target="_blank">
	<input type="hidden" name="hdCodGuia" id="hdCodGuia" value="" />
		
	<table width="580" border="0" cellpadding="0" cellspacing="1">
		<tr>
			<td height="10" bgcolor="#FFFFFF"></td>
		    <td align="center" bgcolor="#FFFFFF" rowspan="3">DOC - Segunda Via da Guia de Pagamento</td>
...
<?php
		...
		$sql_guias = mysql_query("
			...
			INNER JOIN
				doc_des ON doc_des.codigo = guias_declaracoes.codrelacionamento
			INNER JOIN
				cadastro ON cadastro.codigo = doc_des.codopr_credito 
			WHERE
				guia_pagamento.pago = 'N' AND cadastro.cnpj = '$operadora_CNPJ' AND guias_declaracoes.relacionamento = 'doc_des'
			GROUP BY 
				codigo 
			ORDER BY 
				codigo DESC
