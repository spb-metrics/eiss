...
<?php
	...
	if(($mes == "") && ($ano)){
		$stringdata = "AND SUBSTRING(doc_des.competencia,1,4) = '$ano'";
	}elseif(($mes) && ($ano == "")){
		$stringdata = "AND SUBSTRING(doc_des.competencia,6,2) = '$mes'";
	}elseif(($mes) && ($ano)){
		$stringdata = "AND SUBSTRING(doc_des.competencia,1,7) = '$ano-$mes'";
	}
	if($estado){
		$stringestado = "AND doc_des.estado = '$estado'";
	}
	$sql_des = mysql_query("
		SELECT 
...
<table width="1000" bordercolor="#000000" border="0" style="font-family:Arial, Helvetica, sans-serif; font-size:12px; border-width:thin;">
	<tr>
    	<td colspan="12">Institui&ccedil;&atilde;o Financeira: <?php echo $nome;?></td>
    </tr>
    <tr>
        <td colspan="12">N&uacute;mero de declara&ccedil;&otilde;es feitas: <?php echo mysql_num_rows($sql_des);?></td>
    </tr>
    <?php
	if($estado){
	?>
...
if(mysql_num_rows($sql_des)){
    ...
    <tr>
        ...
        <td width="83" align="center">Data/Hora</td>
    </tr>
    <tr>
    	<td colspan="12"><hr color="#000000" size="2"></td>
    </tr>
