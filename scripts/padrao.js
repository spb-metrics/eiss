...
/**Função criaSpanSenha
*Parametro:
*elemento = nome do elemento da função onload
*Esta função cria um span ao lado do input de senha para que o mesmo receba uma resposta
*/
...
/**Função ValidaCNPJ
*Parametros:
*obj = campo com o cnpj/cpf
*idretorno = id do elemento que recebera o retorno da função
*Esta função verifica se o cnpj/cpf que foi digita já existe no DB
...
function acessoAjax(url,form,retorno){	
	...
	/**
	 * verifica o formulario, e para campos em branco retorma um alerta com o title do imput, assim coloca 
	 * <input type="text" name="txtNome" title="Nome do Usuário" />
	 * se tiver vazio vem um alert("Verifique o campo obrigatório: Nome do Usuário");
	 * @author jean
	 */
	function VerificaForm(inputsIds){	
		var msg = "Verifique o campo obrigatório: ";
		var formInputs = inputsIds.split("|");
		
		for(var ind = 0; ind < formInputs.length; ind++){
			//testa se o elemento existe
			if( document.getElementById(formInputs[ind]) ){
				var input = document.getElementById(formInputs[ind]);
				if(!input.value){
					var nomecampo;
					if(input.title){
						nomecampo = input.title;
					}else{
...
//Funcao que ao pressionar o botao faz com que o botao desapareca e um outro campo apareca
function verificaTextArea(textarea)
{
