...
<?php 
	...
	SELECT certidoes_negativas.codigo,
		   ...
		   emissores.nome,
		   IF(emissores.cnpj IS NULL,emissores.cpf,emissores.cnpj),
		   emissores.logradouro,
		   emissores.numero
	FROM certidoes_negativas
	INNER JOIN cadastro as emissores ON certidoes_negativas.codemissor=emissores.codigo
	WHERE (emissores.cnpj='$cnpjcpf_empresa' OR emissores.cpf = '$cnpjcpf_empresa') AND
		   certidoes_negativas.codigo = '$codverif'");	
	list($nrodoc,$codverificacao,$dataemissao,$val,$nomeempresa,$cnpjcpf,$endereco,$end_numero)=mysql_fetch_array($sql);		
	$endereco .= ', '.$end_numero;
