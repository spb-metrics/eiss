...
$sql=mysql_query("
			SELECT 
				...
				emissores.cnpj,
				emissores.cpf,
				emissores.logradouro,
		   		emissores.numero,
				emissores.codigo
			FROM 
				certidoes_pagamento
			INNER JOIN 
				cadastro as emissores ON certidoes_pagamento.codemissor=emissores.codigo
			WHERE 
				(emissores.cnpj='$cnpjcpf_empresa' OR emissores.cpf = '$cnpjcpf_empresa') AND
				certidoes_pagamento.codigo = '$codverif'");	

list($nrodoc,$codverificacao,$certemissao,$certvalidade,$nomeempresa,$cnpjcpf,$cpf,$endereco,$numero,$codemissor)=mysql_fetch_array($sql);
$cnpjcpf .= $cpf;
$endereco .= ', '.$numero;
...
$sql_emissor=mysql_query("SELECT codigo, nome, razaosocial, inscrmunicipal, logradouro, municipio, uf, email 
				  FROM cadastro 
...
if ($resultado){
	...
	if ($existe_emissor){
		$stringSql[0] = ("(
			...
			INNER JOIN
				cadastro as emissores ON emissores.codigo = des.codcadastro
			WHERE
				(emissores.cnpj = '$tomador_CNPJ' OR emissores.cpf = '$tomador_CNPJ') AND
				guia_pagamento.pago = 'S'  AND guias_declaracoes.relacionamento = 'des'
		)"); 
	}
	if ($existe_tomador) {
		$stringSql[2] = ("
			...
			INNER JOIN
			  cadastro as tomadores ON tomadores.codigo = des_issretido.codcadastro
			WHERE
			  (tomadores.cnpj = '$tomador_CNPJ' OR tomadores.cpf = '$tomador_CNPJ') AND
			  guia_pagamento.pago = 'S'  AND guias_declaracoes.relacionamento = 'des_issretido')"); 
	}
